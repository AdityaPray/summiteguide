...
# Endpoint baru untuk suplai data cuaca ke grafik interaktif di dashboard.html
# ==============================================================================
#
# Import yang dibutuhkan (kalau belum ada di app.py):
# from weather_service import get_forecast_by_name, get_history_by_name, get_weather_db
#
# Catatan: endpoint ini HANYA baca dari cache MongoDB (weather_forecast /
# weather_history), tidak memanggil OpenWeatherMap/Open-Meteo langsung.
# Jadi response-nya cepat dan tidak makan quota API eksternal.
...
def api_dashboard_mountains():
    ...
    names = db["weather_forecast"].distinct("mountain_name")
    # Judul yang enak dibaca (Title Case) buat ditampilkan di dropdown,
    # tapi value yang dikirim tetap versi ternormalisasi (lowercase)
...
from weather_service import get_update_logs
...
def cron_update_weather():
    """
    [DIPERBARUI] Sekarang memanfaatkan summary yang dikembalikan
    update_all_mountains_weather() — summary ini JUGA sudah otomatis
    dicatat oleh weather_service.py ke collection terpisah
    `weather_update_logs`, jadi tidak perlu logging manual di sini.
    """
    ...
    try:
        ...
        # update_all_mountains_weather sudah mencatat status "error" ke Mongo
        # sebelum exception ini dilempar, jadi di sini cukup teruskan errornya.
        return jsonify({
...
def api_cron_update_weather_logs():
    ...
    collection terpisah `weather_update_logs`. Berguna untuk monitoring
    tanpa perlu buka MongoDB Atlas langsung — mis. dipakai admin dashboard
    untuk menampilkan "Update cuaca terakhir: ... (sukses/gagal)".
    """
